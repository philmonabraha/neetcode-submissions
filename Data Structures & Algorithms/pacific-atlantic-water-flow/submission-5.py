class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        for i in range(len(heights)):
            atlantic.add((i,len(heights)-1))
        for i in range(len(heights[0])):
            atlantic.add((len(heights[0])-1, i))
        ...
        def dfs(r,c, visited, preval):
            ...
            if r not in range(len(heights)) or r not in range(len(heights[0])) or heights[r][c] < preval or (r,c) in visited:
                return
